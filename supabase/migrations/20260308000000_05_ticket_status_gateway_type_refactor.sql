-- ============================================================
--  BUDDYTICKET — TICKET STATUS & GATEWAY TYPE REFACTOR
--  Renames ONGATE_PENDING → PENDING in ticket_status
--  Renames CASH_DESK → ONGATE in gateway_type
--  Depends: 01, 02, 03, 04 migrations already applied
-- ============================================================

-- Step 1: Add PENDING to ticket_status enum (replaces ONGATE_PENDING)
ALTER TYPE ticket_status ADD VALUE IF NOT EXISTS 'PENDING';

-- Step 2: Add ONGATE to gateway_type enum (replaces CASH_DESK)
ALTER TYPE gateway_type ADD VALUE IF NOT EXISTS 'ONGATE';

-- Step 3: Migrate existing ticket data
UPDATE tickets
  SET status = 'PENDING'
  WHERE status = 'ONGATE_PENDING';

-- Step 4: Migrate existing transaction gateway data
UPDATE transactions
  SET gateway = 'ONGATE'
  WHERE gateway = 'CASH_DESK';

-- Step 5: Update finalize_order_tickets() to use new enum values
...
BEGIN
    ...
    -- 3-way payment source → ticket status mapping
    -- PAYMENT_GATEWAY = paid online  → ACTIVE
    ...
    -- gateway_type mapping for 3 payment sources
...
-- NOTE: Old enum values (ONGATE_PENDING in ticket_status, CASH_DESK in gateway_type)
-- cannot be dropped in PostgreSQL without recreating the type. They remain in the enum
