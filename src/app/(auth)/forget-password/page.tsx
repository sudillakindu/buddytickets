...
import { forgotPassword as forgotPasswordAction, resetPassword as resetPasswordAction, validateResetToken } from '@/lib/actions/auth';
...
const AuthInput = memo(({ icon: Icon, type = 'text', placeholder, value, onChange, focused, onFocus, onBlur, autoComplete, rightElement }: AuthInputProps) => (
...
function ForgotPasswordForm() {
  ...
  const handleResetPassword = useCallback(async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await resetPasswordAction(resetToken, { password: newPassword, confirmPassword });
      if (result.success) {
      ...
      <div className="relative z-10 w-full max-w-md rounded-3xl p-8 sm:p-10 flex flex-col items-center overflow-hidden my-auto bg-white/85 backdrop-blur-xl shadow-[0_25px_50px_-12px_hsl(222.2_47.4%_11.2%_/_0.15),0_0_0_1px_hsl(222.2_47.4%_11.2%_/_0.05)]">
        ...
        {step === 'email' && (
          <form onSubmit={handleSendOtp} className="w-full flex flex-col gap-4">
            <AuthInput icon={Mail} type="email" placeholder="Email Address" autoComplete="email" value={email} onChange={setEmail} focused={focused('email')} onFocus={() => setFocusedField('email')} onBlur={() => setFocusedField(null)} />
        ...
        {step === 'resetPassword' && (
          <form onSubmit={handleResetPassword} className="w-full flex flex-col gap-4">
            <AuthInput 
              ...
              rightElement={
                <Button type="button" variant="ghost" size="icon" onClick={() => setShowNewPassword((p) => !p)} className="absolute right-1.5 top-1/2 -translate-y-1/2 h-8 w-8 hover:bg-transparent text-[hsl(215.4,16.3%,46.9%)] transition-colors duration-200" aria-label={showNewPassword ? 'Hide password' : 'Show password'}>
            ...
            <AuthInput 
              ...
              onBlur={() => setFocusedField(null)} 
              rightElement={
                <Button type="button" variant="ghost" size="icon" onClick={() => setShowConfirmPassword((p) => !p)} className="absolute right-1.5 top-1/2 -translate-y-1/2 h-8 w-8 hover:bg-transparent text-[hsl(215.4,16.3%,46.9%)] transition-colors duration-200" aria-label={showConfirmPassword ? 'Hide password' : 'Show password'}>
