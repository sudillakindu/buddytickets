        ...
        >
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <motion.div
                    ...
                    style={{ background: 'linear-gradient(to right, hsl(222.2 47.4% 11.2% / 0.2), hsl(270 70% 50% / 0.2), hsl(330 80% 60% / 0.2))', x: springX, y: springY }}
                ...
                <motion.div
                    ...
                    style={{ background: 'linear-gradient(to right, hsl(210 100% 60% / 0.2), hsl(180 70% 50% / 0.2), hsl(160 70% 45% / 0.2))', x: springX, y: springY }}
                ...
                {PARTICLES.map((particle) => (
                    <motion.div
                        ...
                        style={{ background: `hsl(${particle.hue}, 70%, 50%)`, left: particle.left, top: particle.top }}
            ...
            <motion.div className="hidden md:block absolute top-1/4 left-4 w-2.5 h-2.5 rounded-full bg-gradient-to-r from-[hsl(222.2,47.4%,11.2%)] to-[hsl(270,70%,50%)]" animate={{ y: [0, 20, 0], scale: [1, 1.2, 1] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} />
            <motion.div className="hidden md:block absolute bottom-1/4 right-4 w-3 h-3 rounded-full bg-gradient-to-r from-[hsl(210,100%,60%)] to-[hsl(180,70%,50%)]" animate={{ y: [0, -20, 0], scale: [1, 1.2, 1] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} />
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="max-w-2xl w-full text-center relative z-10">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 0.8 }}>
                    <motion.span
                        className="font-primary inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium mb-4 bg-white/50 backdrop-blur-sm border border-[hsl(222.2,47.4%,11.2%)]/20 text-[hsl(222.2,47.4%,11.2%)] shadow-sm"
                        initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} whileHover={{ scale: 1.05 }}
                    >
                        <Clock className="w-3.5 h-3.5 text-orange-500" />
                        System Update in Progress
                    </motion.span>
                    <h2 className="font-primary text-3xl sm:text-4xl font-bold text-gray-900 mt-4 tracking-tight">We'll be right back soon!</h2>
                    <p className="font-secondary text-gray-500 mt-3 mb-8 text-base leading-relaxed max-w-lg mx-auto">
                        BuddyTickets is currently undergoing scheduled maintenance to improve your experience. Please grab a cup of coffee and check back in a few minutes.
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Button asChild variant="outline" className="font-primary relative group overflow-hidden inline-flex items-center justify-center gap-2 h-12 px-8 text-sm rounded-xl border-2 transition-all duration-300 shadow-sm hover:shadow-md hover:bg-gray-50 text-[hsl(222.2,47.4%,11.2%)] border-[hsl(222.2,47.4%,11.2%)]/20 w-full sm:w-auto">
                            <a onClick={() => window.location.reload()} className="cursor-pointer w-full">
