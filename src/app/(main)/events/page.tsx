...
export default function EventsPage() {
  ...
  const filteredEvents = useMemo(
    () =>
      events.filter((e) =>
        activeTab.statuses.includes(e.status as EventStatus),
      ),
    [events, activeTab],
  );
          ...
          {FILTER_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "font-secondary text-sm font-semibold px-4 py-1.5 rounded-full border transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(270,70%,50%)]",
                  isActive
