...
export const AnimatedBackground: React.FC<AnimatedBackgroundProps> = memo(
  ({ springX, springY, variant = "compact" }) => {
    const isHero = variant === "hero";
    const blobSize = isHero ? "w-[50vw] h-[50vw]" : "w-[80vw] h-[80vw]";
    const blobSizeSecondary = isHero
      ? "w-[50vw] h-[50vw]"
      : "w-[70vw] h-[70vw]";
    const blurClass = isHero ? "blur-[80px]" : "blur-[60px]";
    const particleSize = isHero ? "w-2 h-2" : "w-1.5 h-1.5";
    ...
    return (
      ...
      >
        <motion.div
          className={`absolute top-[-5%] right-[-30%] ${blobSize} min-w-[200px] min-h-[200px] rounded-full ${blurClass} opacity-30`}
          style={{
            background:
        ...
        />
        <motion.div
          className={`absolute bottom-[-10%] left-[-30%] ${blobSizeSecondary} min-w-[200px] min-h-[200px] rounded-full ${blurClass} opacity-30`}
          style={{
            background:
