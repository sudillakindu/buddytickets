...
export async function getVerifyEmailData(
  ...
  try {
    ...
    if (otpRecord) {
      ...
      const canResendAt =
        new Date(otpRecord.last_sent_at).getTime() + cooldownMs;
      const now = Date.now();
      if (now < canResendAt) {
        canResend = false;
...
export async function resetPassword(
  ...
  try {
    ...
    const { data: consumedToken, error: markErr } = await getSupabaseAdmin()
      .from("auth_flow_tokens")
      .update({ is_used: true })
      .eq("token_id", flowToken.token_id)
      .eq("is_used", false)
    ...
    const { data: user } = await getSupabaseAdmin()
      ...
      .single();
    if (user?.name)
      sendPasswordChangedEmail(flowToken.email, user.name).catch((err) =>
        logger.error({
          fn: "resetPassword",
